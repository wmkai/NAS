# python train.py \
bash ./distributed_train_multi_node.sh 8 \
--aa 'rand-m9-mstd0.5' \
--amp \
--apex-amp \
--aug-splits 0 \
...
--min-lr 1e-05 \
--mixup 0.2 \
--model 'hess_supernet_fix_backbone' \
--model-ema \
...
--clip-grad 1.0 \
--sync-bn \
--drop-path-only-last-two-stages \
--num-arch-training 4 \
--sandwich-rule \
--post-bn-calibration-batch-num 64 \
--warmup-epochs 20 \
--experiment 'hess' \
--eval-metric 'max_net_top1' \
--log-wandb \
