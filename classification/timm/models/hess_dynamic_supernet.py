...
from timm.models.modules.dynamic_layers import DynamicMBConvLayer, DynamicConvBnActLayer, DynamicInjectionMultiSum, DynamicClsHead
from timm.models.modules.static_layers import MobileInvertedResidualBlock, PyramidPoolAgg, TransformerBlock 
from timm.models.modules.transformer import DynamicMlp, DynamicAttention
from timm.models.modules.nn_utils import int2list
...
def _cfg(url='', **kwargs):
    return {
        ...
        'first_conv': 'tpm.stem', 'classifier': 'fc',
        **kwargs
    }

default_cfgs = {
    'hess_supernet_base_fix_backbone': _cfg(),
    'hess_supernet_base_fix_trans': _cfg(),
    'hess_supernet_small': _cfg(),
    'hess_supernet_tiny': _cfg()
}
...
class HESSDynamicSupernet(BaseModule):
    def __init__(self,
        ...
        self.supernet = Config(supernet) # supernet是yml中的supernet_config
        ...
        assert drop_rate == self.drop and drop_path_rate == self.drop_path_rate, 'drop_rate and drop_path_rate in .sh file must match the configs in supernet'
        ...
        if self.supernet.SIM.injection:
            ...
            #             inj_module(self.supernet.SIM.channels[i], self.supernet.SIM.out_channels[i],\
            #                  norm_cfg=self.supernet.SIM.norm_cfg, activations=self.supernet.SIM.act_func))
            for i in range(len(self.supernet.embed_out_indice)):
                if i in self.supernet.SIM.decode_out_indices:
                    # block_cfg = getattr(self.supernet, self.stage_names[self.embed_out_indice[i]+1])
                    # inp = block_cfg.c
                    self.SIM.append( # SIM模块的激活函数默认用h_sigmoid，不受传入参数的影响
...
def _create_hess_dynamic_supernet(variant, pretrained=False, **kwargs): 
    return build_model_with_cfg(HESSDynamicSupernet, variant, pretrained, **kwargs)

# fix backbone model
@register_model
def hess_supernet_fix_backbone(pretrained=False, **kwargs):
    supernet=dict(
    ...
    return _create_hess_dynamic_supernet('hess_supernet_base_fix_backbone', pretrained, **model_args)

# fix trans model
@register_model
def hess_supernet_base_fix_trans(pretrained=False, **kwargs):
    supernet=dict(
        stem=dict(c=[16, 24], act_func='relu6', s=2),
        embed_out_indice=[1, 2, 3, 4],
        c2t_stride=1,
    ...
    return _create_hess_dynamic_supernet('hess_supernet_base_fix_trans', pretrained, **model_args)

@register_model
def hess_supernet_small(pretrained=False, **kwargs):
    supernet=dict(
    ...
    return _create_hess_dynamic_supernet('hess_supernet_small', pretrained, **model_args)

@register_model
def hess_supernet_tiny(pretrained=False, **kwargs):
    supernet=dict(
        stem=dict(c=[16], act_func='relu6', s=2),
        embed_out_indice=[1, 2, 3, 4],
        c2t_stride=1,
        ...
        mb2=dict(
            # c=16, d=2, k=3, t=4,3
            c=[16, 24], d=[1, 2, 3], k=[3, 5], t=[3, 4, 5], s=2,
            act_func='relu6', se=False
        ),
        mb3=dict(
            # c=32, d=2, k=5, t=3
            c=[24, 32, 40], d=[1, 2, 3], k=[3, 5], t=[2, 3, 4, 5], s=2,
            act_func='relu6', se=False
        ),
        mb4=dict(
            # c=64, d=2, k=3, t=3
            c=[56, 64, 72, 80], d=[1, 2, 3], k=[3, 5], t=[2, 3, 4, 5], s=2,
            act_func='relu6', se=False
        ),
        mb5=dict(
            # c=96, d=2, k=5, t=6
            c=[88, 96, 104, 112], d=[1, 2, 3], k=[3, 5], t=[4, 5, 6, 7], s=2,
            act_func='relu6', se=False
        ),
        trans1=dict(
            num_heads=[2, 4, 6, 8], # head 6
            key_dim=[12, 14, 16, 18, 20], # 16
            attn_ratio=[1.6, 1.8, 2.0, 2.2, 2.4], # 2.0
            mlp_ratio=[1.6, 1.8, 2.0, 2.2, 2.4], # 2.0
            d=[1, 2], # 1
    ...
    return _create_hess_dynamic_supernet('hess_supernet_tiny', pretrained, **model_args)
