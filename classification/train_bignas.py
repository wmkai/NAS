...
# attentivenas
import timm.utils.loss_ops as loss_ops 
...
# AttentiveNAS
group = parser.add_argument_group('AttentiveNAS parameters')
group.add_argument("--drop-path-only-last-two-stages", action='store_true', default=True)
group.add_argument("--num-arch-training", default=4, type=int)
group.add_argument("--sandwich-rule", action='store_true', default=True)
# group.add_argument("--grad_clip_value", type=float, default=0.1) # args有clip-grad属性了
group.add_argument("--post-bn-calibration-batch-num", default=64, type=int)
...
def main():
    ...
    model = create_model(
        ...
        drop_rate=args.drop, # 0.2
        drop_connect_rate=args.drop_connect,  # DEPRECATED, use drop_path
        drop_path_rate=args.drop_path, # 0.1
        drop_block_rate=args.drop_block, # 默认None
        global_pool=args.gp,# 默认None
        bn_momentum=args.bn_momentum, # 默认None
        bn_eps=args.bn_eps, # 默认None
    ...
    # 新增
    soft_criterion = loss_ops.KLLossSoft().cuda()
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics = train_one_epoch(
                epoch, model, loader_train, optimizer, train_loss_fn, soft_criterion, args,
            ...
            eval_metrics = validate(model, loader_train, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast)
            ...
            if model_ema is not None and not args.model_ema_force_cpu:
                ...
                ema_eval_metrics = validate(
                    model_ema.module, loader_train, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast, log_suffix=' (EMA)')
...
def train_one_epoch(
        epoch, model, loader, optimizer, loss_fn, soft_criterion, args,
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        # drop_path_only_last_two_stages = getattr(args, 'drop_path_only_last_two_stages', True)
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            model.module.sample_max_subnet()
            model.module.set_dropout_rate(args.drop, args.drop_path) #dropout for supernet
        else:
            model.sample_max_subnet()
            model.set_dropout_rate(args.drop, args.drop_path) #dropout for supernet
        ...
        model.module.set_dropout_rate(0, 0) if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.set_dropout_rate(0, 0) #reset dropout rate
        for arch_id in range(1, num_subnet_training):
            if arch_id == num_subnet_training-1 and sandwich_rule:
                model.module.sample_min_subnet() if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.sample_min_subnet()
            else:
                model.module.sample_active_subnet() if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.sample_active_subnet()

            with amp_autocast():
                output = model(input)
                loss_kd = soft_criterion(output, soft_logits)
            # 新增
            if loss_scaler is not None:
                loss_scaler._scaler.scale(loss_kd).backward(create_graph=second_order)
            else:
                loss_kd.backward(create_graph=second_order)
        ...
        if loss_scaler is not None:
            loss_scaler(
                ...
                create_graph=second_order, need_backward=False) # backward在每一步中已经实现过了
        else:
            # loss.backward(create_graph=second_order) # 在每一步中已经实现过了
...
def validate(model, loader_train, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    with torch.no_grad():
        for net_id in subnets_to_be_evaluated:
            ...
            for batch_idx, (input, _) in enumerate(loader_train):
                if batch_idx >= args.post_bn_calibration_batch_num:
                    break
                input = input.cuda(non_blocking=True)
                subnet(input)
            
            subnet_eval_metrics = validate_one_subnet(subnet, net_id, loader, loss_fn, args, amp_autocast=amp_autocast, log_suffix=log_suffix)
            # subnet_eval_metrics.update([(net_id + '_' + k, v) for k, v in subnet_eval_metrics.items()]) # 把所有key前面加上网络id
            eval_metrics.update(subnet_eval_metrics)
    return eval_metrics



def validate_one_subnet(subnet, net_id, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    batch_time_m = utils.AverageMeter()
    losses_m = utils.AverageMeter()
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if args.local_rank == 0 and (last_batch or batch_idx % args.log_interval == 0):
                log_name = net_id + ' test' + log_suffix
                _logger.info(
                    '{0}: [{1:>4d}/{2}]  '
    ...
    metrics = OrderedDict([(net_id + '_loss', losses_m.avg), (net_id + '_top1', top1_m.avg)]) # wandb和csv里不记录top5
