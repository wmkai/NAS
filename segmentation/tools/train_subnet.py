...
def main():
    ...
    supernet = build_segmentor(
        cfg.model,
        train_cfg=cfg.get('train_cfg'),
        test_cfg=cfg.get('test_cfg'))
    supernet.init_weights() # 初始化模型参数，init_weights函数是定义在model中的

    # 从超网获得子网
    supernet.set_active_subnet(
        width=cfg.subnet.width, 
        depth=cfg.subnet.depth, 
        kernel_size=cfg.subnet.kernel_size, 
        expand_ratio=cfg.subnet.expand_ratio, 
        num_heads=cfg.subnet.num_heads, 
        key_dim=cfg.subnet.key_dim, 
        attn_ratio=cfg.subnet.attn_ratio, 
        mlp_ratio=cfg.subnet.mlp_ratio, 
        transformer_depth=cfg.subnet.transformer_depth,
    )
