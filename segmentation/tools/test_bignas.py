...
from mmseg.utils import get_root_logger, build_ddp, build_dp, get_device, setup_multi_processes
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default='a0',
        help='Model name in “pareto_models” of test config file.')
...
def main():
    ...
    subnet_cfg = cfg.pareto_models[args.model_name] # config文件中的架构名
    model.set_active_subnet( # dynamic_model
        width = subnet_cfg.width,
        depth = subnet_cfg.depth,
        kernel_size = subnet_cfg.kernel_size,
        expand_ratio = subnet_cfg.expand_ratio,
        # out_dim = subnet_cfg.out_dim, 
        num_heads = subnet_cfg.num_heads, 
        key_dim = subnet_cfg.key_dim, 
        attn_ratio = subnet_cfg.attn_ratio, 
        mlp_ratio = subnet_cfg.mlp_ratio, 
        transformer_depth = subnet_cfg.transformer_depth
    )
    subnet_cfg = model.get_active_subnet_settings()
    model = model.get_active_subnet() # static_model
    if fp16_cfg is not None:
        wrap_fp16_model(model)    
    if rank == 0:
    ...
    # build train_loader for bn_calibration
    ...
    if not distributed:
        ...
        bn_calibration(model, train_loader, cfg.post_bn_calibration_batch_num)
